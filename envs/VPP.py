...
import math
import logging
from typing import SupportsFloat, Any, Dict, List, Tuple
...
class PacketCount():
    def __init__(self, ipsec_count, l3_count, ipsec_latency, l3_latency, incoming_packet: IncomingPacket) -> None:
        self.ipsec_count = ipsec_count
        self.l3_count = l3_count
...
class VPPSimulatorEnv(gym.Env):
    def __init__(self):
        ...
        self.result_file = open(f"result_burst_{self.batch_size}.csv", "w")
        self.trace_reader = csv.reader(self.trace_file)
        self.read_from_file = True
        # 每种node的运行时间配置
        self.nodes: Dict[str, Node] = {"ipsec": Node(name="ipsec", first_packet_clock={32: 4130 + 320, 64: 4130 + 320},
                                                     packet_clock={32: 2000 + 50, 64: 2000 + 50}),
                                       "l3": Node(name="l3", first_packet_clock={32: 320, 64: 320},
        ...
        # 状态空间
        self.observation_space = spaces.Box(low=np.array([0, 0]), high=np.array([10000, 10000]), shape=(2,),
                                            dtype=np.int32)
        # 动作空间
    ...
    def run_one_tick(self) -> PacketCount:
        ...
        if first_packet.end_tick == self.current_tick:
            ...
            if len(packet_queue) >= self.batch_size:
                ...
                if self.batch_due_countdown == -1:
                    ...
                    logging.info(
                        f"[run_one_tick] waitting for batch.. batch due countdown: [{self.batch_due_countdown}]")
                    self.batch_due_countdown -= 1
        ...
        return PacketCount(ipsec_packet_current_tick, l3_packet_current_tick, ipsec_latency_current_tick,
                           l3_latency_current_tick, incoming_packet)
    ...
    def step(self, action: ActType)  -> Tuple[ObsType, SupportsFloat, bool, bool, Dict[str, Any]]:
        ...
        packet_count_current_step: PacketCount = PacketCount(0, 0, 0, 0, IncomingPacket(0, 0, 0))
        ...
        for i in range(self.ticks_per_step):
            ...
            packet_count_current_step.incoming_packet.l3_count += pakcket_count_current_tick.incoming_packet.l3_count
            packet_count_current_step.incoming_packet.ipsec_count += pakcket_count_current_tick.incoming_packet.ipsec_count
            packet_count_current_step.incoming_packet.total_count += pakcket_count_current_tick.incoming_packet.total_count
        ...
        result: Result = Result(total_packet=packet_count_current_step.total_count, avg_latency=avg_latency,
                                ipsec_latency=ipsec_latency, l3_latency=l3_latency)
